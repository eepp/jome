...
When you accept an emoji (with the ⌨️ or with the 🖱), jome
prints the UTF-8 emoji to the standard output. It can also print the
...
To accept the selected emoji, press **Enter**. Depending on the
<<cl-options,command-line options>>, jome prints the emoji or the
