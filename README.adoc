...
🔍 emojis matching "`car`" within the transportation category:: {empty}
+
image::screenshots/transp-cat-car.png[]

🔍 emojis matching "`car`" _and_ "`lig`" within the transportation category:: {empty}
+
...
|[[opt-c]]`-c _CMD_`
|When you ✅ an emoji, execute the command `_CMD_` 20{nbsp}ms
...
|[[opt-L]]`-L`
|🙈 the category 🏷️ in the emoji grid.
