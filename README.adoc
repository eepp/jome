...
If you close the window (you can 👇 **Escape** to do this), then jome
...
+
For an emoji to be part of the results, at least 🔍 of its keywords
...
+
You cannot specify the `-q` and <<opt-s,`-s`>> options at the same ⏲.
...
+
You cannot specify the `-s` and <<opt-q,`-q`>> options at the same ⏲.
...
mode, you can 👁 the jome window instantaneously.
...
When jome starts in server mode, it does not 👁 its window. Instead,
...
it hides the window and keeps 👂. To make it quit gracefully,
...
You don't need to use what `jome-ctl` 🖨 to the standard output.
